...
- OpenAI/Codex 主线路径下，`image_caption_model` 统一固定为 `gpt-5.5`，并且 `image_caption` 必须复用全局 OpenAI `api_key/api_base_url`，不再使用独立 caption key/base 分叉。
- 设置页展示值必须和后端实际执行值一致；禁止出现“展示是 A、运行时自动归一化成 B”的隐式改写。
...
- 可编辑 PPTX 导出支持 `export_mode`：`fast / balanced / high_fidelity`。`fast` 速度优先（可关闭视觉结构分析与图标透明抠图）；`high_fidelity` 质量优先（保留视觉结构分析、图标抠图和更细粒度阈值）；`balanced` 使用项目常规配置。
...
### 2026-07-02 05:40 - OpenAI 模型执行值与配置展示值强一致改造

- 范围：`backend/models/settings.py`、`backend/controllers/settings_controller.py`、`backend/app.py`、`backend/services/ai_providers/__init__.py`。
- 动作：在 OpenAI/Codex 路径下把 `image_caption_model` 统一对齐到 `gpt-5.5`；`image_caption_model_source` 与文本来源对齐；`image_caption_api_key/api_base_url` 改为复用全局 OpenAI 配置，不再走独立 caption 凭据分叉；启动加载、设置更新、配置同步和 provider 工厂全部按同一规则收敛。
- 结果：消除“设置展示与实际执行不一致”的根因，不再出现配置显示为 `gemini-*` 但运行时被隐式改写的情况；可编辑 PPTX 视觉结构分析主路径稳定使用 `gpt-5.5` + 全局 OpenAI key/base。
- 计划状态：Phase 7 中“设置页展示模型与后端实际调用一致”验收项已落地。
- 验证：`FULL_TEST_ENV=1 uv run --python 3.13 pytest backend/tests/unit/test_api_settings_provider.py backend/tests/unit/test_app_factory.py backend/tests/unit/test_file_parser_service.py -q` 通过（25 passed）。
- 遗留：前端 `build:check` 仍存在仓库历史 TS 基线问题，非本次改造引入，需单独基线治理。

### 2026-07-02 05:55 - 历史数据库模型字段一次性清洗对齐

- 范围：`/Users/xuran/ppt/banana-slides/backend/instance/database.db`（主仓库运行库）。
- 动作：批量清洗历史配置：`image_caption_model=gemini-* -> gpt-5.5`；并将 `image_caption_model_source` 批量对齐为 `text_model_source`（为空回退 `openai`）。
- 结果：库内存量配置与当前执行策略一致，避免历史脏数据导致新会话/旧任务行为漂移。
- 计划状态：历史数据治理完成，运行时与持久化层一致。
- 验证：更新后复查 `settings`/`user_settings`，`image_caption_model like 'gemini-%'` 剩余 0；`image_caption_model_source` 与目标来源不一致剩余 0。
- 遗留：后续若新增配置导入通道，必须复用同一清洗策略，防止旧值回灌。

### 2026-07-02 06:20 - 可编辑 PPTX 导出质量与速度第一轮优化

- 范围：`backend/services/image_editability/service.py`、`backend/services/export_service.py`、`backend/controllers/export_controller.py`、`backend/services/task_manager.py`、`frontend/src/api/endpoints.ts`。
- 动作：增强图标透明抠图触发与 alpha 质量门槛；渲染前增加视觉图层 IoU 去重，缓解图标重复堆叠；视觉结构分析改为并行执行；可编辑导出参数补充 `export_mode/min_image_size/min_image_area/enhance_quality` 并贯通到任务执行。
- 结果：针对“图标不透明、图标多层堆叠、导出慢、复杂图形还原不足”四类问题完成第一轮工程化修复与参数化治理。
- 计划状态：后端双档/三档导出能力完成，进入前端交互接入阶段。
- 验证：后端单测同批次通过（25 passed）。
- 遗留：复杂图形还原度仍需基于真实业务样例持续调参；端到端对比和自动校正闭环仍需补齐。

### 2026-07-02 06:45 - 前端可编辑导出弹窗接入“极速/平衡/高保真”

- 范围：`frontend/src/pages/SlidePreview.tsx`。
- 动作：在可编辑导出弹窗新增 `export_mode` 三档选择并透传到 `exportEditablePPTX` 请求；补充中英文模式说明；极速模式下前端禁用“图标透明背景”并给出强制策略提示，确保界面行为与后端模式一致。
- 结果：用户可显式选择导出策略，不再只能依赖隐式后端默认；速度优先与质量优先在交互层可感知。
- 计划状态：前后端模式贯通完成（后端策略 + 前端选择 + API 透传）。
- 验证：本次改动已在 `SlidePreview` 请求链路确认 `export_mode` 入参；前端 `npm run -s build:check` 报错仍为仓库历史 TS 基线问题（非本次新增）。
- 遗留：建议补充任务面板对 `export_mode` 的展示，便于用户回看导出结果与模式对应关系。
