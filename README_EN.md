...
<p>
  ...
  <b>Go from idea to presentation in minutes—no tedious formatting, just natural language revisions. Step into the era of "Vibe PPT".</b>
</p>
<p>
...
- **[2026-04-25]**: Asset Toolbox launched — Added three new modes: Full-image editing, Selection editing (overlay/replace), and Smart erasure, based on existing asset generation, providing a unified entry point for a one-stop experience.
...
- **[2026-04-25]**: Supports saving custom text style description templates. These can be named, color-coded, and persistently reused, eliminating the need for repeated manual input.
- **[2026-04-23]**: Supported the gpt-image-2 model. Additionally, the editable background effect for exports has been improved due to model capability upgrades (Select "Generative Retrieval" in Settings - Export Options - Background Retrieval).
...
- **[2026-02-09]**: New Features and Optimizations
  * New Features
    * Supports pasting images on the homepage, outline, and description cards for instant recognition, providing an enhanced interaction experience.
    * Manual Outline Section Editing: Supports manually adjusting which section (part) a page belongs to.
  ...
  * Fixes and Experience Optimizations
    * Fixed export-related 500 errors, reference file association timing, outline/page data misalignment, task polling for incorrect projects, infinite polling for description generation, memory leaks in image previews, and handling of partial failures in batch deletion.
    * Optimized format example prompts, HTTP error message copy, Modal closing experience, clearing of localStorage for old projects, and removal of redundant prompts when creating a project for the first time.
    * Various other optimizations and fixes.

## ✨ Project Origin

Have you ever found yourself in this predicament: you have a presentation tomorrow, but your slides are still a total blank; your mind is full of brilliant ideas, but your enthusiasm is drained by the tediousness of layout and design?
...
- 3️⃣ Finished products look strikingly similar, leading to severe homogenization.
- 4️⃣ Low-quality assets that lack specificity.
- 5️⃣ Disconnected text and image layouts with a poor sense of design.

These flaws make it difficult for traditional AI PPT generators to simultaneously satisfy our two major needs for PPT production: "speed" and "aesthetics." Even those claiming to be "Vibe PPT" are, in my eyes, still far from being truly "Vibe."

However, the emergence of the nano banana🍌 model has changed everything. I tried using 🍌pro to generate PPT pages and found that the results were exceptional in quality, aesthetics, and consistency. Furthermore, it can accurately render almost all text required by the prompt while strictly following the style of reference images. So, why not build a native "Vibe PPT" application based on 🍌pro?

## 👨‍💻 Applicable Scenarios

1. **Beginners**: Rapidly generate beautiful PPTs with zero threshold and no design experience required, reducing the hassle of choosing templates.
2. **PPT Professionals**: Quickly gain design inspiration by referencing AI-generated layouts and combinations of visual elements.
...
- **Natural Language Editing**: Modify outlines or descriptions using "Vibe" style conversational commands (e.g., "Change the third page to a case study"), with real-time adjustments from the AI.
...
- **Partial Redraw**: Make conversational modifications to unsatisfactory areas (e.g., "Change this chart to a pie chart").
...
- **Playback Settings**: Enable slide transition animations before exporting to PPTX. Supports classic effects such as Fade, Push, Wipe, Split, Blinds, Checkerboard, Clock, and more, with support for random application of multiple selections.
- **Perfect Adaptation**: Default 16:9 aspect ratio ensures no layout adjustments are needed for immediate presentation.
...
- **Export images to high-fidelity, clean-background PPT pages with fully editable text and images**
...
- **Convert slides to presentation videos (MP4) with AI voiceovers and subtitles in one click**
...
| Watermark | Free version has watermark | **No watermark, freely add or delete elements** |
...
| ✅ Completed | Vibe voice editing for selected slide areas |
...
1. Deploy and start this application with one click via Rainyun (High bandwidth, suitable for HD image generation and downloads. New users get a 15-day free trial)
...
**(Optional, you can also configure this via the user interface after startup, [click here for the tutorial](https://ziy68cvfvu3.feishu.cn/wiki/GiNawdmpiinSRqkGspocqEWAnkh?from=from_copylink ))** Edit the `.env` file and configure the necessary environment variables:

<details>
<summary>Click to expand details</summary>
  
> **Large model API interfaces in this project follow the AIHubMix platform standard. We recommend using [AIHubMix (click here to access)](https://aihubmix.com/?aff=17EC) to obtain API keys and reduce migration costs.**<br>
> **Friendly Reminder: Google Nano Banana Pro model API costs are relatively high; please be mindful of usage costs.**
```env
...
**Use the new editable export configuration method for better editable export results**: You need to obtain an API KEY from the [Baidu Intelligent Cloud Platform](https://console.bce.baidu.com/iam/#/iam/apikey/list) (click here to enter) and fill it in the `BAIDU_API_KEY` field in the `.env` file (sufficient free usage quota is available). See instructions in https://github.com/Anionex/banana-slides/issues/121 for details.
...
After startup, you can go to **Settings → About → Check for Updates** within the application. The app will determine if an update is available based on the current version SHA; when running from source, the current Git SHA will also be used for this check.
...
You can also check for new versions within the application via **Settings → About → Check for Updates**.
...
This is a sample file used to demonstrate how to translate Markdown content into English. It contains various common Markdown elements.

## 🚀 Core Features

- **Automation**: Supports automatic language detection.
- **High Performance**: Uses efficient algorithms for processing.
- **Extensible**: Supports custom plugins.

## 💻 Code Example

You can use the following code to initialize the project:
...
| :--- | :--- | :--- |
| Translation | ✅ Completed | Supports 50+ languages |
| Documentation | 🏗️ In Progress | Continuously updated |

## 🔗 Related Links

- [Project Homepage](https://github.com/example/project)
- [Issue Feedback](https://github.com/example/project/issues)

---

<p align="center">Powered by Markdown Translation Tool</p>
...
Welcome to follow the author on social media, where I will share information about this project and AI:
